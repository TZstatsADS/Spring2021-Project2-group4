...
uitabs2 <- shinyUI( navbarPage("Weather and Covid", id="nav", theme="bootstrap.css",
                              ...
                              tabPanel("Time Series Visualizations",
                                       fluidRow(column(width=10,
                                                       dateRangeInput("DateRange", "Choose a date range of the time series:", 
...
servertabs2 <- function(input, output) {
  ...
  #date1 <- as.Date(startdate, format ="%m-%d-%Y")
  #date2 <- as.Date(enddate, format="%m-%d-%Y")
  
  #alabama$day <- as.Date(alabama$date, format="%m/%d/%Y")
  
  corona1 <- reactive({alabama %>% dplyr::filter(between(day,  startdate(),  as.Date("2020-03-30"))) })
  
  #corona1 <- alabama %>% dplyr::filter(day >= date1 & day <= date2) 
  ...
  output$plot2 <- renderPlot({
    ...
    #plot(amodel1)
    # test time subsets 
    ggplot()+ geom_line(data = corona1, aes(day,cases), color="blue")
  })
